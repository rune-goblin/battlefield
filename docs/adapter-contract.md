...
`src/app/targeting.ts` adapts an `ActionOffer` and `ActivityOption` to `TargetChoice` objects. `TargetingService.matches` accepts hex, edge, corner and exact-target hits; multiple matches require an explicit choice. `forRef` supports target-first menus. `preview` supplies the icon, affected cells and the projected aim point. `resolve` returns the exact engine action, feedback markers and spell animations, or `null` for an invalid selection. `ActivityAction.target` and `SiegeAction.target` are a `TargetRef`, and an offered target's `id` is its `targetKey`.

Target geometry and effects remain separate. A Burst anchors at a shared corner and affects all cells in its shape. Translocate anchors at the destination, and its engine action names the moved unit and its destination. `src/board/target-point.ts` projects anchors for both DOM markers and PIXI aim lines. The engine remains the authority for legal targets and mechanical effects.
...
Pass ReignMaker's hex fortification tier through unchanged: 1 Earthworks, 2 Wooden Tower, 3 Stone Tower, 4 Fortress. These are the hex tiers from `src/data/fortificationTiers.ts`, rather than the settlement support-building progression. Battlefield offers tiers 1–4 and labels tiers 2 and 3 Wood and Stone. Older saved tier-zero barricades migrate to Earthworks while breaches remain open. Battlefield derives wall durability, hardness, and cover from the tier and generates one closed gate with an explicit interior hex. For live PF2e actors, the card adapter prepares an unsaved clone without ReignMaker’s Fortification effect before reading statistics. PF2e then recalculates circumstance stacking; the source actor stays unchanged. Battlefield applies its own position-dependent Fortified cover. Raw source actors without a clone method retain their source statistics.

Wall state carries remaining boxes, an optional interior cell, and optional gate open state. A painted gate also stores its explicit `facing` cell and `flipped` cycle position. Save and handoff these fields with the board, including `fortInterior` for generated courtyards at the map edge. Fortified is derived from position and wall layout rather than saved on units. Temporary siege fields also persist within a day and clear overnight. Structure-damaging siege abilities currently target walls and gates; future building targets should use the same structural damage calculation.
