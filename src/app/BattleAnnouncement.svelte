...
<style>
  ...
  .message {
    ...
    color: var(--ink); -webkit-text-stroke: 4px var(--paper); paint-order: stroke fill;
  ...
  .round .message {
    font-size: clamp(1.8rem, 3.2vw, 2.8rem); font-weight: 600; letter-spacing: .08em;
    white-space: nowrap;
    animation: round-title 3.6s ease both;
  }
  ...
  .side .message {
    ...
    font-size: 1rem; font-weight: 650; letter-spacing: .18em; text-transform: uppercase;
    animation: side-label 2.4s ease both;
  }
  /* Starts as the 3.6s round title begins its descent at 70%. */
  .side.after-round .message { animation-delay: 2.4s; }
  .side .message::before, .side .message::after { width: 1.7rem; flex-basis: 1.7rem; }
  ...
  @keyframes side-label {
    0% { opacity: 0; visibility: visible; clip-path: inset(-.5rem 100% -.5rem -.5rem); transform: translateX(-14px); }
    25%, 75% { opacity: 1; visibility: visible; clip-path: inset(-.5rem); transform: translateX(0); }
