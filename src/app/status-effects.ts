...
const TEXT: Record<Status, (u: Unit, state: BattleState) => string> = {
  guard: (u) => `+${u.guard!.defence} Defence until this unit next activates.${u.guard!.cap ? ' Dug in: every hit against it caps at one wound.' : ''}${u.guard!.holds ? ' Under cover: it holds its ground against an Overrun.' : ''}`,
  ...
  persistent: (u) => `1 more wound at the end of its next activation, then Fortitude against DC ${u.persistent!.dc} or 1 disorder, as any wound.`,
  ...
  stoneskin: () => 'Every hit against it caps at one wound and costs it no disorder, until it has next acted.',
  'sure-strike': () => 'Its next attack rolls twice and keeps the better. Spent by that attack, or at the end of its next activation.',
  wrath: () => 'Its next hit deals persistent damage: 1 more wound at the end of the target\'s next activation.',
