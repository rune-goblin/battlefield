...
import { act, availableActions, createBattle, isRouted, refOf, targetKey, unit } from '../engine/index.js';
...
describe('action commitment', () => {
  it('buys attack accuracy, spends the full cost, and retains one attack per activation', () => {
    const s = act(setup(true), { unit: 'u0', type: 'fight', activity: 1, target: { kind: 'unit', ids: ['u2'] }, focus: 1 }, scriptedRng([11, 20]));
    expect(checkFor(s, 'Strike against').modifier).toBe(13);
    expect(unit(s, 'u2').wounds).toBe(1);
    expect(unit(s, 'u0').actions).toBe(1);
    expect(() => act(s, { unit: 'u0', type: 'fight', activity: 1, target: { kind: 'unit', ids: ['u2'] } }, scriptedRng([20]))).toThrow(/already attacked/);
    const full = act(setup(true), { unit: 'u0', type: 'fight', activity: 1, target: { kind: 'unit', ids: ['u2'] }, focus: 2 }, scriptedRng([10, 20]));
  ...
  it('supports a focused tier-two shot without changing its suppression effect', () => {
    ...
    const s = act(state, { unit: 'u0', type: 'shoot', activity: 2, target: { kind: 'unit', ids: ['u2'] }, focus: 1 }, scriptedRng([11, 20]));
  ...
  it('rejects unsupported, malformed and unaffordable commitment', () => {
    for (const focus of [-1, .5, 3, NaN]) {
      expect(() => act(setup(true), { unit: 'u0', type: 'fight', activity: 1, target: { kind: 'unit', ids: ['u2'] }, focus }, scriptedRng([10]))).toThrow(/commitment/);
    }
    expect(() => act(setup(), { unit: 'u0', type: 'guard', activity: 1, focus: 1 }, scriptedRng([10]))).toThrow(/commitment/);
    expect(() => act(setup(true), { unit: 'u0', type: 'fight', activity: 2, target: { kind: 'unit', ids: ['u2'] }, focus: 2 }, scriptedRng([10]))).toThrow(/needs 4 actions/);
  ...
  it('lets a hasted unit concentrate tier three but caps the extra bonus at +4', () => {
    ...
    const s = act(state, { unit: 'u0', type: 'fight', activity: 3, target: { kind: 'unit', ids: ['u2'] }, focus: 1 }, scriptedRng([11, 20, 20]));
  ...
  it('allows concentrated Missile above the tradition activity cap', () => {
    const state = setup(); unit(state, 'u2').square = parse('c5');
    const s = act(state, { unit: 'u0', type: 'cast', spell: 'blast', activity: 1, target: { kind: 'unit', ids: ['u2'] }, focus: 2 }, scriptedRng([10, 20]));
  ...
  it('uses one boosted Line roll for both enemies, with one wound resolution each', () => {
    ...
    const target = availableActions(state, 'u0').find(o => o.spell === 'blast')!.activities[1].targets.find(t => t.id === targetKey({ kind: 'cell', cells: ['c5', 'c6'] }))!;
    const s = act(state, { unit: 'u0', type: 'cast', spell: 'blast', activity: 2, target: refOf(target), focus: 1 }, scriptedRng([11, 20, 20]));
  ...
  it('concentrates shared Healing and Rally rolls without increasing their scope', () => {
    ...
    const healed = act(state, { unit: 'u0', type: 'cast', spell: 'healing', activity: 2, target: { kind: 'unit', ids: ['u0', 'u1'] }, focus: 1 }, scriptedRng([10]));
  ...
  it('boosts Controlling DC and retains the chosen activity effect', () => {
    ...
    const s = act(state, { unit: 'u0', type: 'cast', spell: 'controlling', activity: 2, target: { kind: 'unit', ids: ['u2'] }, focus: 1 }, scriptedRng([9]));
...
describe('resisting morale pressure', () => {
  it('lets a unit at two disorder resist Press and routes it only on a failed save', () => {
    ...
    const action = { unit: 'u0', type: 'fight' as const, activity: 2 as const, target: { kind: 'unit' as const, ids: ['u2'] } };
  ...
  it('adds no disorder when an Overrun cannot displace the target', () => {
    ...
    for (const [rolls, disorder] of [[[15, 20, 20], 0], [[15, 20, 1], 1]] as const) {
      const s = act(state, { unit: 'u0', type: 'fight', activity: 3, target: { kind: 'unit', ids: ['u2'] } }, scriptedRng([...rolls]));
  ...
  it('uses the same saving throw for cavalry impact, while Stoneskin prevents disorder', () => {
    ...
    const protectedResult = act(protectedState, { unit: 'u0', type: 'fight', activity: 2, target: { kind: 'unit', ids: ['u2'] } }, scriptedRng([20, 1, 1]));
