...
describe('grade derivation', () => {
  it('grades a troop with an empty tactic list — the normal case — on structure alone', () => {
    ...
    expect(gradesFor(rabble)).toEqual({ move: 2, shoot: 1, fight: 2, guard: 1, withdraw: 2, rally: 1, cast: 1 });
  });

  it('a mounted troop charges', () => {
    const camels = troop('Qadiran Camel Corps');
    expect(camels.signals).toContain('mounted');
    expect(gradesFor(camels).move).toBe(3);
  });

  it('a levy is granted the bottom rung of everything but its own two feet', () => {
    const levy = troop('Peasant Levy');
    expect(gradesFor(levy)).toEqual({ move: 2, shoot: 1, fight: 2, guard: 1, withdraw: 2, rally: 1, cast: 1 });
  ...
  it('an elite earns its threes', () => {
    ...
    expect(gradesFor(einherjar)).toMatchObject({ move: 3, fight: 3, guard: 3, withdraw: 3 });
  });
  ...
  it('lets a hand-authored tactic raise a grade the numbers do not reach', () => {
    const knights = troop('Knights');
    expect(knights.tactics).toEqual(['cavalry-charge']);
    expect(gradesFor(knights).move).toBe(3);
    expect(gradesFor({ ...knights, tactics: [] }).move).toBe(2);
  ...
  it('grades every published troop without hand-authoring', () => {
    ...
    expect(spread.filter((n) => n > 1).length).toBeGreaterThanOrEqual(6);
  });
});
