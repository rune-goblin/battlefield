...
import type { ActivityAction, HealingChoice } from '../engine/types.js';
...
const cast = (tree: Tree, target: string, healingChoices?: Record<string, HealingChoice>): ActivityAction => ({ type: 'cast', unit: 'u0', spell: tree, activity: 4, target, healingChoices });
...
describe('caster progression', () => {
  ...
  it('gives apprentices distinct trees and rejects spells before their unlock without rolling', () => {
    ...
    expect(() => act(wizard, { ...cast('blast', 'u3'), activity: 2 }, noRoll)).toThrow(/not available/);
    expect(() => act(wizard, { ...cast('healing', 'u1'), activity: 1 }, noRoll)).toThrow(/not available/);
  ...
  it('lets an apprentice concentrate a basic spell while limiting spell commitment under Haste', () => {
    ...
    const next = act(s, { ...cast('blast', 'u3'), activity: 1, focus: 2 }, scriptedRng([10, 20]));
    expect(next.log.find(e => e.text.includes('Missile against'))?.check?.modifier).toBe(15);
    const master = field(); unit(master, 'u0').haste = 2;
    expect(() => act(master, { ...cast('blast', option(master, 'blast').targets[0].id), focus: 1 }, noRoll)).toThrow(/commitment is at most three/);
...
describe('mastery spells', () => {
  it('Storm resolves one shared attack against four enemies and spends three actions', () => {
    ...
    const target = option(s, 'blast').targets.find(t => t.id.split('+').length === 4 && ['e5','f5','f6','g6'].every(cell => t.id.split('+').includes(cell)))!;
    expect(target).toBeDefined();
    const next = act(s, cast('blast', target.id), scriptedRng([10, 20, 20, 20, 20]));
    ...
    expect(() => act(next, { ...cast('blast', 'u3'), activity: 1 }, noRoll)).toThrow(/already attacked/);
  ...
  it.each([[1,3,2],[5,2,2],[11,1,1],[20,0,0]])('Renewal roll %i restores the correct Health and Morale', (die, wounds, disorder) => {
    ...
    const next = act(s, cast('healing','u1'), scriptedRng([die]));
  ...
  it('Renewal clears the selected conditions in priority order and respects a one-condition success', () => {
    ...
    const action = cast('healing','u1',{u1:{conditions:[...choices.u1.conditions]}});
  ...
  it('Sanctuary protects its caster through the next activation and shares Stoneskin expiry', () => {
    const s=field('divine');
    let next=act(s,cast('defense','u0+u1'),noRoll);
  ...
  it('publishes a tier-IV spell event and separate saving throw events', () => {
    ...
    const action = cast('controlling', 'u3+u4+u5');
  ...
  it('Gate transfers two units, offers single transfers, and rejects shared or occupied destinations', () => {
    ...
    const target=option(s,'movement').targets.find(t=>t.id==='e3+e4+d3+d4')!;
    expect(target).toBeDefined();
    ...
    expect(pick.target?.id).toBe('d3+d4');
    ...
    for(const bad of ['e3+e4+d3+e4','e3+d3','e3+e8']) expect(()=>act(s,cast('movement',bad),noRoll)).toThrow(/not a target/);
  });
});
