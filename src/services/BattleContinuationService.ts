...
import type { DicePort, MintPort } from '../runtime/ports.js';
...
function nextSpec(battle: BattleState, changes: Partial<BoardSpec>, mint: MintPort): BoardSpec {
  const base = battle.nextBoard?.spec ?? { ...battle.board.spec, construction: null, seed: mint.seed() };
  return {
...
export function createBattleContinuationService(
  { dice, mint }: { dice: DicePort; mint: MintPort },
): BattleContinuationService {
  return {
    declareRecovery: (session, side, choices, userId) => {
      ...
      const declared = submitTo(session, 'night.recovery', side, choices.map((c) => ({ ...c })), userId, mint);
    ...
    answerSurrender: (session, side, accept, userId) => {
      ...
      return withBattle(closeInteraction(submitTo(session, 'day.surrender', side, accept, userId, mint), 'day.surrender'), answered);
    },
    ...
    chooseBattlefield: (session, spec) => {
      ...
      const nextBoard = spec === null ? null : generateBoard(nextSpec(battle, spec, mint));
    ...
    declareDeployment: (session, side, positions, userId) => {
      ...
      return submitTo(session, 'nextDay.deployment', side, { ...positions }, userId, mint);
    },
  };
}
