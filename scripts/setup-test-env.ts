...
//
// The module under test is the exception: `modules/battlefield` is linked to this repo's
// `dist-foundry`, so a fresh `npm run build:foundry` reaches the harness with no re-clone. It
// holds no LevelDB, so the link shares no lock.
import {
  existsSync, mkdirSync, copyFileSync, writeFileSync, unlinkSync, lstatSync, rmSync,
  readFileSync, readdirSync, readlinkSync, renameSync, symlinkSync,
} from 'node:fs';
...
const TEST_WORLD = process.env.TEST_WORLD ?? 'stolen-lands';
...
// A sibling module under development (ReignMaker, say) may symlink `packs` back to its repo's
// built LevelDB. Left alone, the test instance locks that repo's packs and collides with the
// developer's own Foundry.
...
// Linked here whatever the real install holds (a dev symlink, a deployed copy, or nothing), so
// the harness always serves this checkout's build.
const moduleDest = join(dataDir, 'modules', MODULE_ID);
const distFoundry = join(repo, 'dist-foundry');
removePath(moduleDest);
mkdirSync(distFoundry, { recursive: true });
symlinkSync(distFoundry, moduleDest);
console.log(`🔗 modules/${MODULE_ID} → ${distFoundry}`);
